...
class tableAdmin(admin.ModelAdmin, ExportExcelMixin, AdminConfirmMixin):
    ...
    list_display = ('name', 'status', 'server_ip', 'ssh_port', 'user', 'model', 'sn', 'cpu', 'memory', 'disk', 'os',
                    'deployment_plan','doployment_mod', 'remote_card_ip', 'remote_card_user', 'organization',
                    'old_asset_number', 'new_asset_number', 'owner', 'owner_department', 'is_use', '_type',
                    'cost_center', 'checker', 'create_time', 'update_time', 'location', 'commnet')

    # list_display_links = ('server_ip',)

    search_fields = ('server_ip', 'sn', 'owner', 'checker', 'is_use', 'remote_card_ip', 'new_asset_number')
    ...
    def has_view_permission(self, request, obj=None):
        if request.user.is_superuser:
            self.fieldsets = [
                (None, {'fields': ['name', 'server_ip', 'ssh_port', 'user', 'model', 'sn', 'cpu', 'memory', 'disk',
                                   'os', 'deployment_plan', 'doployment_mod', 'remote_card_ip', 'remote_card_user',
                                   'organization', 'old_asset_number', 'new_asset_number', 'owner', 'owner_department',
                                   'is_use', '_type', 'cost_center', 'checker', 'location', 'status']}),
            ...
            self.fieldsets = [
                (None, {'fields': ['name', 'server_ip', 'ssh_port', 'user', 'model', 'sn', 'cpu', 'memory', 'disk',
                                   'os', 'deployment_plan', 'doployment_mod', 'remote_card_ip', 'remote_card_user',
                                   'organization', 'old_asset_number', 'new_asset_number', 'owner', 'owner_department',
                                   'is_use', '_type', 'cost_center', 'checker', 'location']}),
                ('密码', {'fields': ['password', 'remote_card_password'], 'classes': ['collapse']}),
            ]
        else:
            self.fieldsets = [
